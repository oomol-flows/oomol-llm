...
class Request:
  ...
  def post(self) -> Any:
    with urllib.request.urlopen(self._request, timeout=self._timeout) as response:
      ...
      try:
        ...
        raise ValueError(f"Failed to decode JSON response") from err
  ...
  def _assert_ok(self, response: Any, content_type: str):
    if response.getcode() not in _POST_OK_STATUS:
      raise Exception(f"request LLM failed with status code {response.getcode()}")
    content_types = response.headers.get("Content-Type", ())
    if isinstance(content_types, str):
      content_types = (content_types,)
    if content_type not in content_types:
      content_types = ", ".join(content_types)
      raise Exception(f"Invalid Content-Type {content_type}: expect {content_types}")